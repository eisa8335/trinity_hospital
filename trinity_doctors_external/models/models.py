...
class TrinityDoctorExternal(models.Model):
    _name = 'trinity.doctor.external'
    _description = 'Trinity Doctors External'
    _rec_name = 'doctor_id'
    ...
    def _restrict_duplicates(self):
        for rec in self:
            duplicates = self.env['trinity.doctor.external'].search(
    ...
    #     result = []
    #     for rec in self:
    #         name = f"{rec.doctor_id} {rec.first_name}"
